// Attack is the time that it takes for the instrument to fade in
// (typically .002 seconds).  Decay reflects the behavior of how
// the sound dies or silences over time.
// All the sounds are synthesized, which gives a lot of flexibility.
// This library can produce these sounds at ANY frequency, and any BPM
// The wave equations represent the diferent shifted waves that
// make up the sounds.  These are basically Fourier coefficients
  ...
  acoustic: {
    ...
    wave: function(i, sampleRate, frequency) {
      ...
      if (valueTable.length <= Math.ceil(period)) {
        ...
        valueTable[playVal] = (valueTable[playVal >= (
          valueTable.length - 1) ? 0 : playVal + 1] +
          valueTable[playVal]) * 0.5;
        if (playVal >= Math.floor(period)) {
          if (playVal < Math.ceil(period)) {
            if ((periodCount % 100) >= p_hundredth) {
              // Reset
              resetPlay = true;
              valueTable[playVal + 1] = (valueTable[0] +
                valueTable[playVal + 1]) * 0.5;
              vars.periodCount++;
            }
          } else {
            resetPlay = true;
